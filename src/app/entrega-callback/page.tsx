...
const MelhorEnvioCallback = () => {
    ...
    const calculateShipment = useCallback(
        async (token: string) => {
    ...
    useEffect(() => {
        const fetchAccessToken = async () => {
            ...
            const existingToken = Cookies.get('melhorenvio_token');
            const refreshToken = Cookies.get('melhorenvio_refresh_token');
            ...
            if (existingToken && !tokenIsValid && refreshToken) {
                try {
                    ...
                    Cookies.set('melhorenvio_token', newAccessToken, {
                        expires: expires_in / (60 * 60 * 24),
                        path: '/',
                        sameSite: 'strict',
                    });

                    Cookies.set('melhorenvio_refresh_token', newRefreshToken, {
                        expires: 30, // 30 days
                        path: '/',
                        sameSite: 'strict',
                    });
            ...
            if (!existingToken && code) {
                try {
                    ...
                    if (response.status === 201 || response.status === 200) {
                        ...
                        Cookies.set('melhorenvio_token', access_token, {
                            expires: expires_in / (60 * 60 * 24),
                            path: '/',
                            sameSite: 'strict',
                        });
    ...
    const isTokenValid = () => {
        const tokenExpiry = Cookies.get('melhorenvio_token_expiry');
